...
import { Clock, Flame, Users, Eye, Bookmark } from "lucide-react"
      ...
      <Card className="group hover:shadow-xl transition-all duration-300 border-0 bg-white/80 backdrop-blur-sm overflow-hidden h-full flex flex-col">
        ...
        <CardContent className="space-y-4 flex-1 flex flex-col">
          ...
          <div className="space-y-2 flex-1">
            <h4 className="font-medium text-gray-900 text-sm">{t("recipe.keyIngredients")}</h4>
            <div className="flex flex-wrap gap-1">
              ...
              {recipe.ingredients.length > 3 && (
                <Badge variant="outline" className="text-xs">
                  {t("recipe.moreIngredients", { count: recipe.ingredients.length - 3 })}
                </Badge>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center justify-between pt-2 mt-auto">
            ...
            <Link href={`/recipe/${recipe.id}`}>
              <Button size="sm" className="bg-green-600 hover:bg-green-700 text-white flex-shrink-0">
                ...
                <span className="hidden sm:inline">{t("recipe.viewRecipe")}</span>
